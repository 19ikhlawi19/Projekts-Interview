...
def extract_image_archives():
    ...
    wenn der Zielordner (config.LINES_FOLDER) leer oder nicht vorhanden ist.
    """
    tgz_folder = config.IAM_TGZ_FOLDER
    output_folder = config.LINES_FOLDER # Ziel ist der Bildordner selbst
...
def validate_data_and_filter(all_relative_paths, all_labels_encoded):
    ...
    valid_paths = []
    valid_labels = []
    img_root = config.LINES_FOLDER
    max_len = config.TARGET_MAXLEN
    skipped_path = 0
    skipped_len = 0
...
def create_offline_splits(force_new=False):
    ...
    """
    train_file = config.TRAIN_LABELS_LINES_FILE
    val_file = config.VAL_LABELS_LINES_FILE
    test_file = config.TEST_LABELS_LINES_FILE
    xlsx_file = config.LINES_XLSX_PATH
    image_root = config.LINES_FOLDER
    ...
    for rel_path_raw, text_label in zip(all_relative_paths_raw, all_transcripts_raw):
         rel_path = utils.clean_relative_path(rel_path_raw)
    ...
    if skipped_char_errors > 0:
        ...
        logger.error(f"  BITTE config.py -> CHAR_LIST entsprechend anpassen!")
    ...
    train_s, val_s, test_s = config.TRAIN_SPLIT, config.VAL_SPLIT, config.TEST_SPLIT

    try:
        # Test Split
        if test_s > 0 and len(X) > 1:
            test_size_abs = max(1, int(test_s * len(X))) # Mindestens 1 Sample
            X_rem, X_test, y_rem, y_test = train_test_split(X, y, test_size=test_size_abs, random_state=config.RANDOM_SEED, shuffle=True)
        ...
        if val_s > 0 and len(X_rem) > 1:
            ...
            if val_size_abs > 0:
                X_train, X_val, y_train, y_val = train_test_split(X_rem, y_rem, test_size=val_size_abs, random_state=config.RANDOM_SEED, shuffle=True)
        ...
        success = True
        if X_train: success &= utils.save_split_data(X_train, y_train, 'train', vectorizer)
        else: logger.warning("Trainings-Split ist leer! Nichts zu speichern.")
        if X_val: success &= utils.save_split_data(X_val, y_val, 'val', vectorizer)
        else: logger.info("Validierungs-Split ist leer (oder wurde nicht angefordert).")
        if X_test: success &= utils.save_split_data(X_test, y_test, 'test', vectorizer)
...
def prepare_offline_data(force_splits=False):
    ...
    if not os.path.exists(config.LINES_XLSX_PATH):
        logger.error(f"Primäre Datenquelle {config.LINES_XLSX_PATH} nicht gefunden."); return False
