...
public class MainboardContainmentCheck extends MainboardCheck {
  @Override
  ...
  public void check(final Mainboard model) {
    final Function1<MainboardId, IdentifiableElement> _function = (MainboardId it) -> {
      return it.getElement();
    };
    final Consumer<Container> _function_1 = (Container it) -> {
      this.check(it);
    };
    Iterables.<Container>filter(ListExtensions.<MainboardId, IdentifiableElement>map(this.adapter.getEntityIds(), _function), Container.class).forEach(_function_1);
    final Function1<MainboardId, IdentifiableElement> _function_2 = (MainboardId it) -> {
      return it.getElement();
    };
    final Consumer<GraphModel> _function_3 = (GraphModel it) -> {
      this.checkModel(it);
    };
    Iterables.<GraphModel>filter(ListExtensions.<MainboardId, IdentifiableElement>map(this.adapter.getEntityIds(), _function_2), GraphModel.class).forEach(_function_3);
  }
