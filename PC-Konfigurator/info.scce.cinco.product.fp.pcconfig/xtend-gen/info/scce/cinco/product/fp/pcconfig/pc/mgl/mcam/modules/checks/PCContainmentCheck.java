...
public class PCContainmentCheck extends PCCheck {
  @Override
  ...
  public void check(final PC model) {
    final Function1<PCId, IdentifiableElement> _function = (PCId it) -> {
      return it.getElement();
    };
    final Consumer<Container> _function_1 = (Container it) -> {
      this.check(it);
    };
    Iterables.<Container>filter(ListExtensions.<PCId, IdentifiableElement>map(this.adapter.getEntityIds(), _function), Container.class).forEach(_function_1);
    final Function1<PCId, IdentifiableElement> _function_2 = (PCId it) -> {
      return it.getElement();
    };
    final Consumer<GraphModel> _function_3 = (GraphModel it) -> {
      this.checkModel(it);
    };
    Iterables.<GraphModel>filter(ListExtensions.<PCId, IdentifiableElement>map(this.adapter.getEntityIds(), _function_2), GraphModel.class).forEach(_function_3);
  }
