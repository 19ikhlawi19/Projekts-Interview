...
# Finale Größenanpassung/Padding geschieht in utils.load_image!
...
def preprocess_folder(input_folder, output_folder):
    ...
    for file_path_in in progress_bar:
        ...
        utils.ensure_dir(os.path.dirname(file_path_out))
        ...
        try:
            ...
            if original_image is not None:
                ...
                if processed_image is not None:
                    save_success = utils.save_image(processed_image, file_path_out)
...
def preprocess_all_image_data():
    """ Führt preprocess_folder für den in config.py definierten Ordner aus. """
    input_folder = config.LINES_FOLDER
    ...
    logger.info("-> Passen Sie ggf. 'config.LINES_FOLDER' an, um diesen Ordner zu nutzen.")
