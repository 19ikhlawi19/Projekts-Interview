...
import config
import utils
# ÄNDERUNG: Importiere aus utils statt training_offline
from utils import OfflineHandwritingDataset, collate_fn, load_data_for_ctc
import model_offline_ocr
# Entferne setup_logging Import, Logging wird von main.py initialisiert
    ...
    if num_samples == 0:
        ...
        for i in indices:
            try:
                ...
                if config.WORDS_FOLDER and not os.path.isabs(img_path_relative):
                    img_path_abs = os.path.join(config.WORDS_FOLDER, img_path_relative)
...
def plot_error_distribution(all_true, all_pred, split_name, base_path=None):
    ...
    try:
        ...
        cer_values = [utils.levenshtein_distance(pr, tr) / max(1, len(tr))
                      for pr, tr in zip(all_pred, all_true) if tr]
        wer_values = [utils.compute_wer(pr, tr)
    ...
    try:
        ...
        for i in indices:
            ...
            try:
                ...
                # Lade Bild über utils.load_image
                image = utils.load_image(img_path_relative)
...
def plot_metrics_comparison(all_split_metrics, base_path=None):
    ...
    try:
        ...
        metrics_to_plot = [m for m in config.METRICS_TO_COMPUTE if m in common_metrics and m != 'loss']
...
def evaluate_model_on_split(model, data_loader, criterion, device, split_name):
    ...
    Verwendet utils.compute_metrics für die Metrikberechnung.
    ...
    # Annahme: DataLoader wurde mit utils.OfflineHandwritingDataset erstellt
    ...
    utils.save_metrics_to_file(metrics, split_name, config.METRICS_PATH)

    # Erstelle Visualisierungen für diesen Split im Unterordner
    results_path_split = os.path.join(config.RESULTS_PATH, split_name)
...
def load_train_val_splits():
    ...
    """
    train_paths, train_labels = utils.load_split_data(config.TRAIN_LABELS_FILE)
    val_paths, val_labels = utils.load_split_data(config.VAL_LABELS_FILE)
    ...
    if not train_paths or not val_paths:
        ...
        all_paths, all_labels_encoded = utils.load_data_for_ctc(config.WORDS_CSV, config.WORDS_FOLDER)
        if not all_paths:
...
def load_test_split():
    ...
    Gibt Pfade und *kodierte* Labels zurück.
    """
    test_paths, test_labels = utils.load_split_data(config.TEST_LABELS_FILE)
    ...
    if not test_paths:
        ...
        logger.error(f"Test-Split-Datei {config.TEST_LABELS_FILE} nicht gefunden oder leer.")
        return [], [] # Leere Listen zurückgeben
...
def evaluate_offline_model():
    ...
    if not os.path.exists(best_model_path):
        ...
        base_checkpoint_dir = os.path.dirname(config.CHECKPOINT_PATH) # z.B. .../checkpoints/
        ...
        if latest_version_dir:
             ...
             config.MODEL_VERSION = os.path.basename(latest_version_dir)
             config.CHECKPOINT_PATH = latest_version_dir
             ...
             logger.info(f"Ausgabe-Pfade auf Version '{config.MODEL_VERSION}' gesetzt.")
        else:
    ...
    try:
        train_loader = DataLoader(
            ...
            batch_size=config.BATCH_SIZE, shuffle=False, collate_fn=utils.collate_fn, num_workers=num_workers, pin_memory=pin_memory
    ...
    # Verwende utils.save_metrics_to_file, das Serialisierung handhaben sollte
    utils.save_metrics_to_file(valid_metrics, "evaluation_summary", config.METRICS_PATH)

    # Erstelle Vergleichsplots
    plot_metrics_comparison(valid_metrics, config.RESULTS_PATH)
    ...
    if os.path.exists(history_path):
        try:
             ...
             final_summary_path = os.path.join(config.METRICS_PATH, "final_summary_metrics.json")
    ...
    report = {
        ...
        'dataset_info': {
            'source_csv': config.WORDS_CSV,
            'word_image_folder': config.WORDS_FOLDER,
            'split_files': {
                'train': config.TRAIN_LABELS_FILE,
                'val': config.VAL_LABELS_FILE,
                'test': config.TEST_LABELS_FILE,
            },
            'sizes': {
        ...
        'hyperparameters_at_eval_time': { # Könnten sich von Trainingszeit unterscheiden
             ...
             'char_list_size': len(config.CHAR_LIST),
    ...
    report_path = os.path.join(config.RESULTS_PATH, "evaluation_report.json")
    try:
...
if __name__ == "__main__":
    ...
    if not logging.getLogger().hasHandlers():
         logger = setup_logging() # Verwende die Funktion aus diesem Modul oder importiere sie
