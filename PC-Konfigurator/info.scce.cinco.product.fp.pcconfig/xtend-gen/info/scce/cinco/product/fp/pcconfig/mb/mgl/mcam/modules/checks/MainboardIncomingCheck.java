...
public class MainboardIncomingCheck extends MainboardCheck {
  @Override
  ...
  public void check(final Mainboard model) {
    ...
    Iterables.<Node>filter(ListExtensions.<MainboardId, IdentifiableElement>map(this.adapter.getEntityIds(), _function), Node.class).forEach(_function_1);
  }
  
  public void check(final Node node) {
    EList<ConnectionConstraint> _incomingConstraints = node.getIncomingConstraints();
    for (final ConnectionConstraint it : _incomingConstraints) {
      {
        ...
        if (_not) {
          ...
          String _plus_1 = (_plus + " of INCOMING [");
  ...
  public void print(final ConnectionConstraint it) {
