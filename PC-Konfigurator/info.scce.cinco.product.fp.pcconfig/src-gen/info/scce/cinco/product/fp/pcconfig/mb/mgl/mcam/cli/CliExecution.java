...
public class CliExecution {
	...
	private MainboardExecution fe = new MainboardExecution();
	...
	private void runMerge(String origPath, String localPath, String remotePath,
		...
		MainboardAdapter mergeModel = null;
		try {
		...
		CompareProcess<MainboardId, MainboardAdapter> localCompare = null;
		try {
		...
		CompareProcess<MainboardId, MainboardAdapter> remoteCompare = null;
		try {
			remoteCompare = fe
		...
		MergeProcess<MainboardId, MainboardAdapter> mp = null;
		try {
		...
		CheckProcess<MainboardId, MainboardAdapter> cp = null;
		try {
	...
	private void runCompare(String origPath, String localPath) {
		...
		try {
			CompareProcess<MainboardId, MainboardAdapter> compare = fe
	...
	private void runCheck(String filePath) {
		...
		try {
			...
			CheckProcess<MainboardId, MainboardAdapter> check = fe
			...
			ArrayList<MainboardAdapter> models = new ArrayList<>();
			models.add(model1);
