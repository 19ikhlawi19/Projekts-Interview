...
import config # Importiere zentrale Konfiguration
...
def set_seeds(seed=config.RANDOM_SEED): # Verwende Seed aus config
...
def get_device(force_gpu=False):
    ...
    effective_force_gpu = force_gpu or (config.DEVICE.lower() == 'cuda') # Berücksichtige config
    device_name = config.DEVICE.lower() # Default aus config
...
def get_project_root():
    """Finds the project root directory (assuming config.py is at the root)."""
    try:
        ...
        if not os.path.exists(os.path.join(project_root, 'config.py')):
             ...
             if os.path.exists(os.path.join(project_root_from_file, 'config.py')):
                   logger.warning(f"Project root derived from utils.py location: {project_root_from_file} instead of config.PROJECT_ROOT")
...
class VectorizeChar:
    ...
    """
    def __init__(self, max_len=config.TARGET_MAXLEN):
        ...
        if hasattr(config, 'CHAR_LIST') and isinstance(config.CHAR_LIST, list) and config.CHAR_LIST:
            base_chars = list(config.CHAR_LIST)
            logger.info(f"Verwende CHAR_LIST aus config.py ({len(base_chars)} Zeichen).")
        else:
            logger.warning("config.CHAR_LIST nicht definiert, ungültig oder leer. Verwende Standard ASCII (letters+digits+punct+space).")
...
def load_image(absolute_image_path):
    """
    Lädt ein Bild, konvertiert zu Graustufen, passt Höhe an config.IMG_HEIGHT an,
    padded auf config.PAD_TO_WIDTH, normalisiert auf [0,1] und gibt es als
...
def save_split_data(relative_paths, encoded_labels_list, split_name, vectorizer):
    ...
    if split_name == 'train': output_file = config.TRAIN_LABELS_LINES_FILE
    elif split_name == 'val': output_file = config.VAL_LABELS_LINES_FILE
    elif split_name == 'test': output_file = config.TEST_LABELS_LINES_FILE
