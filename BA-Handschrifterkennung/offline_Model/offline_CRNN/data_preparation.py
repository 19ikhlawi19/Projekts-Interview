...
def extract_iam_data():
    """
    Entpackt die IAM-Archive ins Verzeichnis config.DATASET_PATH_OFFLINE.
    Nur relevant wenn IAM 
    """
    if not os.path.exists(config.IAM_TGZ_FOLDER):
        logger.warning("IAM_TGZ_FOLDER existiert nicht. Überspringe Entpacken.")
        return

    archives = [f for f in os.listdir(config.IAM_TGZ_FOLDER) if f.endswith('.tgz')]
    for arc in archives:
        arc_path = os.path.join(config.IAM_TGZ_FOLDER, arc)
        try:
            with tarfile.open(arc_path, 'r:gz') as tar:
                tar.extractall(path=config.DATASET_PATH_OFFLINE)
            logger.info(f"Entpackt: {arc} in {config.DATASET_PATH_OFFLINE}")
...
def parse_iam_words(ascii_dir, output_csv):
    ...
    """
    utils.create_directory(os.path.dirname(output_csv))
...
def prepare_data():
    ...
    extract_iam_data()
    # 2) Falls ASCII-Files 
    if os.path.exists(config.IAM_ASCII_FOLDER):
        # parse_iam_words(config.IAM_ASCII_FOLDER, config.WORDS_CSV)
