...
public class CliExecution {
	...
	private PCExecution fe = new PCExecution();
	...
	private void runMerge(String origPath, String localPath, String remotePath,
		...
		CompareProcess<PCId, PCAdapter> localCompare = null;
		try {
		...
		CompareProcess<PCId, PCAdapter> remoteCompare = null;
		try {
			remoteCompare = fe
		...
		MergeProcess<PCId, PCAdapter> mp = null;
		try {
		...
		CheckProcess<PCId, PCAdapter> cp = null;
		try {
	...
	private void runCompare(String origPath, String localPath) {
		...
		try {
			CompareProcess<PCId, PCAdapter> compare = fe
	...
	private void runCheck(String filePath) {
		...
		try {
			...
			CheckProcess<PCId, PCAdapter> check = fe
			...
			ArrayList<PCAdapter> models = new ArrayList<>();
			models.add(model1);
