...
try:
    # Versuche, RUN_OUTPUT_PATH zu bekommen, wenn config.py geladen werden kann
    try:
        # Temporärer Import nur für Pfad (ohne Logging-Nebenwirkungen in config)
        import config as temp_config
        log_file_path = os.path.join(temp_config.RUN_OUTPUT_PATH, "run.log")
        temp_run_output_path = temp_config.RUN_OUTPUT_PATH # Update für spätere Nutzung
    except Exception as cfg_read_err:
        print(f"[WARNUNG] Konnte Pfad aus config.py nicht lesen ({cfg_read_err}). Verwende temporären Logpfad: {log_file_path}", file=sys.stderr)
...
try:
    import config # Lädt NUR die Variablen
    import utils
...
def main():
    ...
    try:
        logger.info(f"Setze Random Seed auf {config.RANDOM_SEED}")
        utils.set_seeds(config.RANDOM_SEED) # Nutze Funktion aus utils
    ...
    try:
        ...
        utils.ensure_dir(config.RUN_OUTPUT_PATH)
    ...
    parser.add_argument('--create_splits', action='store_true',
                        help="[Schritt 3] Erstellt/lädt Train/Val/Test-Splits aus der Excel-Datei (config.LINES_XLSX_PATH).")
    ...
    parser.add_argument('--lr', type=float, default=None, help=f"Überschreibt config.LEARNING_RATE")
    parser.add_argument('--batch_size', type=int, default=None, help=f"Überschreibt config.BATCH_SIZE")
    parser.add_argument('--accum_steps', type=int, default=None, help=f"Überschreibt config.ACCUMULATION_STEPS")
    parser.add_argument('--epochs', type=int, default=None, help=f"Überschreibt config.EPOCHS")
    parser.add_argument('--img_height', type=int, default=None, help=f"Überschreibt config.IMG_HEIGHT")
    parser.add_argument('--pad_width', type=int, default=None, help=f"Überschreibt config.PAD_TO_WIDTH")
    parser.add_argument('--early_stopping', type=int, default=None, help=f"Überschreibt config.EARLY_STOPPING_PATIENCE")
    parser.add_argument('--target_maxlen', type=int, default=None, help=f"Überschreibt config.TARGET_MAXLEN")
    parser.add_argument('--dropout', type=float, default=None, help=f"Überschreibt config.TRANSFORMER_DROPOUT")
    parser.add_argument('--embed_dim', type=int, default=None, help=f"Überschreibt config.TRANSFORMER_EMBED_DIM")
    ...
    parser.add_argument('--data_root', type=str, default=None, help="Überschreibt Bildordner (config.LINES_FOLDER).")
    ...
    if args.output_dir is not None:
        if os.path.isabs(args.output_dir): # Erlaube absolute Pfade
            full_run_path = args.output_dir
             ...
             full_run_path = os.path.join(config.PROJECT_ROOT, args.output_dir) # Relativ zum Projekt
        ...
        config.PATH_CONFIG = {k: getattr(config, f"{k.upper()}_PATH") for k in ['run_output','model_save','checkpoints','results','plots','metrics']}
        try:
             logger.info(f"Erstelle überschriebenen Ausgabeordner: {full_run_path}")
             utils.ensure_dir(config.RUN_OUTPUT_PATH) # Hauptordner
             for path in config.PATH_CONFIG.values(): utils.ensure_dir(path) # Unterordner
        ...
        config_updated.append(f"RUN_OUTPUT_PATH (override) -> {config.RUN_OUTPUT_PATH}")
    ...
    try:
        final_log_level_str = config.LOGGING_LEVEL.upper()
    ...
    best_model_default = os.path.join(config.CHECKPOINT_PATH, "best_offline_transformer_model.pth") # Korrekter Name
    ...
    if args.prepare_data:
        ...
        try: data_preparation.extract_image_archives(); logger.info("[Schritt 1/5] OK.")
    ...
    if args.create_splits:
        ...
        try:
             success = data_preparation.create_offline_splits(force_new=args.force_new_splits)
    ...
    if args.train:
        ...
        try: training_offline.train_offline_transformer_model(); logger.info("[Schritt 4/5] Training beendet (Details siehe Log).")
    ...
    if args.evaluate:
        ...
        if evaluation_model_path:
            try: evaluation_offline.evaluate_offline_transformer(model_path_to_evaluate=evaluation_model_path); logger.info("[Schritt 5/5] Evaluation beendet.")
    ...
    logger.info(f"Ausgabeordner: {config.RUN_OUTPUT_PATH}")
    logger.info("="*50)
